...
#    })
    ...
    #Delete Bridges
    (requestce_sdk_space.find_bridges().content["bridges"] || []).each do |bridge|
      requestce_sdk_space.delete_bridge(bridge["name"])
    end
    
    #Delete Models
    (requestce_sdk_space.find_bridge_models().content['models'] || []).each do |model|
      puts "Model: #{model["name"]}"
      requestce_sdk_space.delete_bridge_model(model["name"])
    end
    ...
    space['spaceAttributeDefinitions'] =
      JSON.parse(File.read("#{request_ce_dir}/spaceAttributeDefinitions.json").gsub("<space-slug>", space_slug))
    # add team attribute definitions
    space['teamAttributeDefinitions'] =
      JSON.parse(File.read("#{request_ce_dir}/teamAttributeDefinitions.json").gsub("<space-slug>", space_slug))
    # add user attribute definitions
    space['userAttributeDefinitions'] =
      JSON.parse(File.read("#{request_ce_dir}/userAttributeDefinitions.json").gsub("<space-slug>", space_slug))
    # add user profile attribute definitions
    space['userProfileAttributeDefinitions'] =
      JSON.parse(File.read("#{request_ce_dir}/userProfileAttributeDefinitions.json").gsub("<space-slug>", space_slug))
    # add bridges
    space['bridges'] = Dir["#{request_ce_dir}/bridges/bridges/*"].map do |bridge_file|
      JSON.parse(File.read("#{bridge_file}").gsub("<space-slug>", space_slug) ).delete_if { |k,v| k == "key" }
    end
    ...
    Dir["#{request_ce_dir}/teams/*"].each do |team_file|
      ...
      if !requestce_sdk_space.find_team(team['name']).content['team']
        requestce_sdk_space.add_team(team)
      end
    end
...
=end
    ...
    datastore_forms = requestce_sdk_space.find_datastore_forms().content['forms']
    datastore_forms.each do |form|
      puts "######################## deleting datastore #{form['slug']} ######################################################"
      requestce_sdk_space.delete_datastore_form(form['slug'])
    end
    ...
    #Delete kapps
    kapps = requestce_sdk_space.find_kapps().content['kapps']
    kapps.each do |kapp| 
      requestce_sdk_space.delete_kapp(kapp['slug'])
    end
...
puts "#########Begin Bridgehub: #{bridgehub_sdk.inspect}"
  import = false
