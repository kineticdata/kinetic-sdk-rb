...
ce_server
    # Log into the Space with the space_admin_credentials
    requestce_sdk = KineticSdk::RequestCe.new({
      app_server_url: ce_server,
      space_slug: space_slug,
      username: space_admin_credentials["username"],
      password: space_admin_credentials["password"],
...
puts "spaceObjects = #{spaceObjects.inspect}"
    spaceObjects.each do | k, v |
puts "generate file for #{k}"
puts "JSON = #{JSON.pretty_generate(v)}" if k == "spaceAttributeDefinitions"
...
puts "generate file for #{obj['name'].slugify}"
...
puts "generate file #{k}"
    ...
    puts "Stripping out Created/Updated Dates and Users #{space_slug}"
